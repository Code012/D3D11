...
rem set win32_libs=%win32_libs% Winmm.lib
rem set win32_lins=%win32_libs% Xinput.lib 
:: CROSS_PLATFORM DEFINES
rem set defines=	      -DHANDMADE_INTERNAL=1
rem set defines=%defines% -DHANDMADE_SLOW=1
:: important to specify drive, or it can't find file
:: No optimisations (slow)L -Od; all optimisations (fast): -O2
cl -Od %compiler% -DHANDMADE_WIN32=1 %defines% %debug% %code_path%main.cpp %win32_libs% /link %win32_link%
popd